...
# ========================================================
# sing-box 综合管理脚本  v2.4  (Hysteria2强制BBR + 官方仓库安装 + Hysteria2 obfs + AnyTLS padding_scheme)
# ========================================================
...
# ============================================================
# [升级] 手动下载官方 GitHub Release 二进制（仅作为 APT/DNF 官方仓库
# 不可用时的回退方案；成功返回 0，失败返回 1，不在此处 pause）
# ============================================================
install_base_manual() {
    echo -e "${CYAN}>>> 回退为手动下载官方 Release 二进制...${PLAIN}"
    ...
    elif command -v dnf &>/dev/null; then dnf install -y curl jq tar wget util-linux
    else echo -e "${RED}不支持的包管理器${PLAIN}"; return 1; fi
    ...
    [[ -z "$TAG" || "$TAG" == "null" ]] && { echo -e "${RED}无法获取版本号，检查网络或 GitHub API 限制${PLAIN}"; return 1; }
    ...
    wget -q --show-progress -O "$TMP_DIR/sing-box.tar.gz" "$url" || {
        echo -e "${RED}下载失败${PLAIN}"; rm -rf "$TMP_DIR"; return 1; }
    tar -xzf "$TMP_DIR/sing-box.tar.gz" -C "$TMP_DIR" || {
        echo -e "${RED}解压失败${PLAIN}"; rm -rf "$TMP_DIR"; return 1; }

    local BIN; BIN=$(find "$TMP_DIR" -type f -name "sing-box" -executable | head -1)
    [[ -z "$BIN" ]] && { echo -e "${RED}未找到可执行文件${PLAIN}"; rm -rf "$TMP_DIR"; return 1; }
    cp "$BIN" /usr/local/bin/sing-box && chmod +x /usr/local/bin/sing-box
    rm -rf "$TMP_DIR"

    # 手动安装没有发行版打包好的 systemd 单元，需要自建
...
# ============================================================
# [升级] 安装/更新 sing-box
# 优先使用官方 APT / DNF / YUM 仓库（deb.sagernet.org / sing-box.app），
# 好处：官方签名校验、后续可直接 apt/dnf 升级、系统自带 systemd 单元
# （/usr/bin/sing-box + /etc/sing-box/config.json，与本脚本路径一致）。
# 仓库不可用或系统不支持时，自动回退到手动下载 GitHub Release 二进制。
# ============================================================
install_base() {
    echo -e "${GREEN}>>> 检测系统与安装方式...${PLAIN}"
    local method=""

    if command -v apt-get &>/dev/null; then
        echo -e "${CYAN}检测到 APT，尝试通过官方仓库安装/升级 sing-box...${PLAIN}"
        apt-get update -y && apt-get install -y curl jq tar wget uuid-runtime gnupg
...
EOF
        ...
        if apt-get update -y && apt-get install -y sing-box; then
            method="apt"
        else
            echo -e "${RED}✘ 官方 APT 仓库安装失败，回退到手动下载方式${PLAIN}"
        fi

    elif command -v dnf &>/dev/null || command -v yum &>/dev/null; then
        echo -e "${CYAN}检测到 DNF/YUM，尝试通过官方仓库安装/升级 sing-box...${PLAIN}"
        if command -v dnf &>/dev/null; then
            dnf install -y curl jq tar wget util-linux dnf-plugins-core
            ( dnf config-manager --add-repo https://sing-box.app/sing-box.repo 2>/dev/null ) || \
            ( dnf config-manager addrepo --from-repofile=https://sing-box.app/sing-box.repo 2>/dev/null )
            if dnf install -y sing-box; then method="dnf"; fi
        else
            yum install -y curl jq tar wget util-linux yum-utils
            yum-config-manager --add-repo https://sing-box.app/sing-box.repo
            if yum install -y sing-box; then method="yum"; fi
        fi
        [[ -z "$method" ]] && echo -e "${RED}✘ 官方 DNF/YUM 仓库安装失败，回退到手动下载方式${PLAIN}"

    else
        echo -e "${YELLOW}未识别 APT/DNF/YUM 包管理器，使用通用手动下载方式${PLAIN}"
    fi

    if [[ -z "$method" ]]; then
        if install_base_manual; then
            method="manual"
        else
            echo -e "${RED}✘ 安装失败${PLAIN}"; pause; return
        fi
    fi

    # 重新定位可执行文件路径：官方仓库装到 /usr/bin，手动安装在 /usr/local/bin
    SB_BIN=$(command -v sing-box || echo "/usr/local/bin/sing-box")
    ...
    echo -e "${GREEN}✔ 安装完成！版本: ${VER:-未知}   安装方式: ${method}${PLAIN}"
    pause
}
...
add_node() {
    ...
    case $choice in
        ...
        2|3|7|8) # 需要证书的协议
            local p_type def_p usr_json tls_json PASS SNI_NAME ALLOW_INS OBFS_JSON="null"
            case $choice in
            ...
            case "$p_type" in
                ...
                hysteria2)
                    ...
                    local LINK_Q="insecure=$ALLOW_INS&sni=$SNI_NAME"

                    # [升级] Hysteria2 obfs 流量混淆：伪装 QUIC 特征，增强抗 DPI 识别能力
                    echo -e "\n${YELLOW}--- Hysteria2 obfs 流量混淆（可选）---${PLAIN}"
                    echo " 1. 不启用"
                    echo " 2. Salamander（推荐，兼容性最好，客户端普遍支持）"
                    echo " 3. Gecko（更强抗DPI，需 sing-box ≥ 1.14.0 且客户端同样支持）"
                    read -p "选择 (默认 1 不启用): " obfs_choice
                    local OBFS_TYPE=""
                    case "$obfs_choice" in
                        2) OBFS_TYPE="salamander" ;;
                        3) OBFS_TYPE="gecko" ;;
                    esac
                    if [[ -n "$OBFS_TYPE" ]]; then
                        local OBFS_PASS; OBFS_PASS=$(gen_pass)
                        OBFS_JSON=$(jq -n --arg t "$OBFS_TYPE" --arg p "$OBFS_PASS" '{"type":$t,"password":$p}')
                        LINK_Q="${LINK_Q}&obfs=${OBFS_TYPE}&obfs-password=${OBFS_PASS}"
                        echo -e "${GREEN}✔ 已启用 $OBFS_TYPE 混淆，密码: $OBFS_PASS${PLAIN}"
                    fi

                    LINK="hysteria2://$PASS@$IP:$PORT?${LINK_Q}#$TAG" ;;
                trojan)
                    usr_json="[{\"password\":\"$PASS\"}]"
            ...
            make_tmp
            # [升级] Hysteria2 未设置 up_mbps/down_mbps 时显式开启 ignore_client_bandwidth，
            # 强制客户端使用 BBR 拥塞控制，避免退化为旧版 Hysteria CC 导致性能不稳定；
            # 若启用了 obfs 混淆则一并写入
            jq --arg port "$PORT" --arg type "$p_type" --arg tag "$TAG" \
               --argjson users "$usr_json" --argjson tls "$tls_json" --argjson obfs "$OBFS_JSON" \
               '.inbounds += [({"type":$type,"tag":$tag,"listen":"::","listen_port":($port|tonumber),
                 "users":$users,"tls":$tls}
                 + (if $type=="hysteria2" then {"ignore_client_bandwidth":true} else {} end)
                 + (if ($type=="hysteria2" and $obfs != null) then {"obfs":$obfs} else {} end))]' \
               "$CONFIG_FILE" > "$_TMP_JSON"
            ;;
        ...
        9) # AnyTLS
            ...
            # [升级] AnyTLS padding_scheme：流量填充方案，对抗基于包长分布的被动流量识别
            echo -e "\n${YELLOW}--- AnyTLS padding_scheme 流量填充方案（可选）---${PLAIN}"
            echo " 1. 使用内置默认方案（推荐，官方 anytls-go 默认值）"
            echo " 2. 自定义填充方案（逐行输入，回车空行结束）"
            read -p "选择 (默认 1): " pad_choice
            local PADDING_JSON="null"
            if [[ "$pad_choice" == "2" ]]; then
                echo -e "${CYAN}格式参考官方默认（每行一条，直接回车空行结束输入）：${PLAIN}"
                echo "  stop=8"
                echo "  0=30-30"
                echo "  1=100-400"
                echo "  2=400-500,c,500-1000,c,500-1000,c,500-1000,c,500-1000"
                echo "  3=9-9,500-1000"
                echo "  4=500-1000  5=500-1000  6=500-1000  7=500-1000"
                local pad_lines=() pad_line
                while true; do
                    read -p "  第$(( ${#pad_lines[@]} + 1 ))行 (留空结束): " pad_line
                    [[ -z "$pad_line" ]] && break
                    pad_lines+=("$pad_line")
                done
                if [[ ${#pad_lines[@]} -gt 0 ]]; then
                    PADDING_JSON=$(printf '%s\n' "${pad_lines[@]}" | jq -R . | jq -s .)
                    echo -e "${GREEN}✔ 已录入 ${#pad_lines[@]} 行自定义填充方案${PLAIN}"
                else
                    echo -e "${YELLOW}未输入内容，回退为内置默认方案${PLAIN}"
                fi
            fi
    ...
    if [[ -f "$LINK_DIR/${TAG}.link" ]]; then
        ...
        case $TYPE in
            ...
            hysteria2)
                ...
                local OBFS_T OBFS_P OBFS_Q=""
                OBFS_T=$(echo "$CONF" | jq -r '.obfs.type // ""')
                OBFS_P=$(echo "$CONF" | jq -r '.obfs.password // ""')
                [[ -n "$OBFS_T" ]] && OBFS_Q="&obfs=${OBFS_T}&obfs-password=${OBFS_P}"
                echo -e "${BLUE}hysteria2://$PASS@$HOST:$PORT?sni=$SNI${OBFS_Q}#$TAG${PLAIN}" ;;
            shadowsocks)
                local METHOD PASS
...
while true; do
    clear
    echo -e "==============================================="
    echo -e "          ${GREEN}Sing-box 综合管理脚本  v2.4${PLAIN}"
    echo -e "==============================================="
    show_status
    echo -e "-----------------------------------------------"
